...
import { getTotalSiswa, setTotalSiswa, getImageSiswa, setImageSiswa } from "../../api/statCounts";
import { uploadImage } from "../../api/uploadApi";
import StatCard from "../../components/StatCard";
import SiswaDefaultImage from "../../assets/siswa_card.jpg";

function AdminSiswa() {
  const [count, setCount] = useState<number>(1279);
  const [customImage, setCustomImage] = useState<string | null>(null);
  const [uploadingImage, setUploadingImage] = useState(false);
  const [savedSuccess, setSavedSuccess] = useState(false);

  useEffect(() => {
    setCount(getTotalSiswa());
    setCustomImage(getImageSiswa());
  }, []);
  ...
  const activeDisplayImage = customImage || SiswaDefaultImage;

  return (
    <div className="p-8 max-w-5xl">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-800">Manajemen Total & Gambar Siswa</h1>
        <p className="mt-1 text-gray-500">
          Atur total jumlah siswa dan gambar StatCard Siswa yang ditampilkan di beranda sekolah
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-6 md:p-8 shadow-sm">
            <h2 className="text-lg font-bold text-gray-800 mb-2">Total Siswa saat ini</h2>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 mb-6">
              ...
              <div className="flex-1 w-full text-center">
                ...
                />
                <span className="text-xs text-gray-400 mt-1 block">Siswa / Peserta Didik</span>
              </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-6 md:p-8 shadow-sm">
            <h2 className="text-lg font-bold text-gray-800 mb-2">Ubah Gambar StatCard Siswa</h2>
            <p className="text-sm text-gray-500 mb-4">
              Unggah gambar kustom untuk menggantikan gambar bawaan pada kartu Siswa di Beranda.
            </p>
          ...
          {savedSuccess && (
            <div className="p-3 rounded-xl bg-green-50 text-green-700 border border-green-200 text-sm font-medium text-center animate-fade-in">
              ✓ Data Siswa berhasil disimpan dan diperbarui di Beranda!
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-1 bg-[#FFA20D] p-6 rounded-2xl shadow-md text-white sticky top-8">
          ...
          <div className="pointer-events-none transform scale-95">
            <StatCard image={activeDisplayImage} number={count.toString()} label="Siswa" />
          </div>
        </div>
